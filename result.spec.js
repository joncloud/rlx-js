...
exports.fn = async () => {
    ...
    assert.eq(await OkPromise('1').map(text => +text).toSync(), Ok(1));

    // mapErr
    const stringify = x => `error code: ${x}`;
    assert.eq(Ok(2).mapErr(stringify), Ok(2));
    assert.eq(Err(13).mapErr(stringify), Err('error code: 13'));
    assert.eq(await OkPromise(2).mapErr(stringify).toSync(), Ok(2));
    assert.eq(await ErrPromise(13).mapErr(stringify).toSync(), Err('error code: 13'));
    ...
    assert.eq(await OkPromise(2).and(Err('late error')).toSync(), Err('late error'));
    assert.eq(await ErrPromise('early error').and(Ok('foo')).toSync(), Err('early error'));
    assert.eq(await ErrPromise('not a 2').and(Err('late error')).toSync(), Err('not a 2'));
    assert.eq(await OkPromise(2).and(Ok('different result type')).toSync(), Ok('different result type'));
    ...
    assert.eq(await OkPromise(2).andThen(sq).andThen(sq).toSync(), Ok(16));
    assert.eq(await OkPromise(2).andThen(sq).andThen(err).toSync(), Err(4));
    assert.eq(await OkPromise(2).andThen(err).andThen(sq).toSync(), Err(2));
    assert.eq(await ErrPromise(3).andThen(sq).andThen(sq).toSync(), Err(3));
    ...
    assert.eq(await OkPromise(2).or('late error').toSync(), Ok(2));
    assert.eq(await ErrPromise('early error').or(Ok(2)).toSync(), Ok(2));
    assert.eq(await ErrPromise('not a 2').or(Err('late error')).toSync(), Err('late error'));
    assert.eq(await OkPromise(2).or(Ok(100)).toSync(), Ok(2));
    ...
    assert.eq(await OkPromise(2).orElse(sq).orElse(sq).toSync(), Ok(2));
    assert.eq(await OkPromise(2).orElse(err).orElse(sq).toSync(), Ok(2));
    assert.eq(await ErrPromise(3).orElse(sq).orElse(err).toSync(), Ok(9));
    assert.eq(await ErrPromise(3).orElse(err).orElse(err).toSync(), Err(3));
